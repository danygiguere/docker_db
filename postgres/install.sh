...
docker-compose exec docker_postgres psql -U postgres -c "DROP DATABASE IF EXISTS ${DB_DATABASE};"
sleep 5 #Give postgres 5sec to drop database before creating a new one
docker-compose exec docker_postgres psql -U postgres -c "CREATE DATABASE ${DB_DATABASE};"
